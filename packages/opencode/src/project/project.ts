...
export namespace Project {
  ...
  function invalidateProjectCache(...namespaces: string[]) {
    const target = namespaces.length > 0 ? namespaces : ["project"]
  ...
  export async function fromDirectory(directory: string) {
    ...
    const row = await Database.cached(
      `project.row:${data.id}`,
      () => Database.use((db) => db.select().from(ProjectTable).where(eq(ProjectTable.id, data.id)).get()),
  ...
  export function get(id: ProjectID): Info | undefined {
    const row = Database.cachedSync(
      `project:get:${id}`,
      () => Database.use((db) => db.select().from(ProjectTable).where(eq(ProjectTable.id, id)).get()),
      { ttl: 5000, namespace: "project" },
    )
