...
For daemon master-worker development, run validation from the package directory:
...
The `test:daemon-p8` command validates Scenario A-G integration coverage and gate checks (Z/W/L/M/D/S), plus single-file bootstrap and TUI attach compatibility.
